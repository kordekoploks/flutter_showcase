...
class AccountBloc extends Bloc<AccountEvent, AccountState> {
  ...
  void _onLoadAccount(GetAccount event, Emitter<AccountState> emit) async {
    try {
      ...
      cashedResult.fold(
        (failure) => (emit(AccountError(data: state.data, failure: failure))),
        (accounts) {
          if (accounts.isEmpty) {
            emit(AccountEmpty(
              data: accounts,
    ...
    try {
      ...
      cashedResult.fold(
        ...
        (newData) {
          ...
          final updatedData = List<Account>.from(state.data)..add(newData);
  ...
  void _onUpdateAccount(UpdateAccount event, Emitter<AccountState> emit) async {
    try {
      emit(
          AccountUpdating(dataUpdating: event._accountModel, data: state.data));
      ...
      updatedResult.fold(
        ...
        final index =
            state.data.indexWhere((account) => account.id == updatedAccount.id);
    ...
    try {
      ...
      cashedResult.fold(
        ...
        if (updatedData.isEmpty) {
          emit(AccountEmpty(data: updatedData));
        }
      });
    } catch (e) {
      emit(AccountError(
